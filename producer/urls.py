...
urlpatterns = patterns('',
    ...
    #url(r'^avail/(?P<prod_id>\d+)/(?P<year>\d{4})/(?P<month>\d+)/(?P<day>\d+)/$',
    #    "producer.views.producer_avail", name="producer_avail"),
    ...
    url(r'^membersupplydemand/(?P<from_date>\w{10})/(?P<to_date>\w{10})/(?P<member_id>\d+)/$',
        "producer.views.member_supply_and_demand", name='member_supply_demand'),
